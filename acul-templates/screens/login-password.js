/**
 * ACUL Screen: login-password (Login / password step)
 *
 * Auth0's Identifier-First architecture always splits identifier and password
 * entry into two screens — there's no supported way to merge them into login-id
 * (confirmed by Auth0 staff on their community forum). This screen exists so
 * that step gets TravelZero's branding too, instead of falling back to classic
 * Universal Login. The identifier was already collected on login-id and is
 * carried forward via screen.screen.data.username — this screen only needs
 * the password.
 *
 * No experiment branching here: by the time a user reaches this screen they've
 * already committed to the password path, so it always renders in the same
 * "password mode" look — blue accent, carousel on the left — matching login-id's
...
const SLIDES = [
  { src: 'https://images.unsplash.com/photo-1543832923-44667a44c804?w=1920&q=80', label: 'London', caption: 'Long weekends, well spent' },
  { src: 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?w=1920&q=80', label: 'London', caption: 'The Thames at golden hour' },
...
function renderTravelZero(screen) {
  ...
  const editLink = screen?.screen?.editIdentifierLink ?? '#';
  const resetUrl = screen?.screen?.resetPasswordLink ?? '#';
  return `
    <div class="tz-layout">
      <div class="tz-panel">
        <div class="tz-card">
          ...
          <div class="tz-head">
            <h1>Enter your password</h1>
            <p>Signing in as <strong>${username}</strong> · <a href="${editLink}">Not you?</a></p>
          </div>
          <form id="password-form" class="tz-form" novalidate>
            <input type="password" name="password" id="password-main" placeholder="Password" autocomplete="current-password" required autofocus />
