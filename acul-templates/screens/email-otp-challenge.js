/**
 * ACUL Screen: email-otp-challenge (Email OTP verification step)
 *
 * Passwordless signup/login variant. User verifies their email via a 6-digit
 * OTP that Auth0 mailed to them. After successful verification, the transaction
 * advances to the next step (passwordless signup: passkey-enrollment; passwordless
 * login: dashboard).
 *
 * This screen is used by email-OTP passwordless connections. EmailIdentifierChallenge
 * is more generic (could be email or SMS), but EmailOTPChallenge is specifically
 * for email OTP flows.
 *
 * Scoping:
